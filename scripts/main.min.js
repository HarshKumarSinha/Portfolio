...
const observer = new IntersectionObserver(
  ...
  hiddenElements = document.querySelectorAll(".scroll-reveal");
function runHeroEntranceAnimation() {
  (anime.set(".intro h1", { translateZ: 150, rotateX: -20 }),
    anime.set(".social-links .btn", {
      opacity: 0,
      translateY: 20,
      translateZ: 50,
    }),
    anime.set(".profile-img-container", { rotateY: 20 }),
    anime
      .timeline({ easing: "easeOutElastic(1, .8)" })
      .add({
        targets: ".intro h1",
        opacity: [0, 1],
        translateY: [30, 0],
...
(lenis.on("scroll", (e) => {
  if (timelineSection && timelineContainer) {
    const e = timelineContainer.getBoundingClientRect();
    let t = (window.innerHeight / 2 - e.top) / e.height;
    ((t = Math.max(0, Math.min(1, t))),
      timelineContainer.style.setProperty("--line-height", 100 * t + "%"));
  }
}),
...
function openResume(e) {
  e.preventDefault();
  const t = document.getElementById("resumeModal"),
    n = document.getElementById("resumeFrame"),
    i = document.getElementById("mobileResumeContainer"),
    s = window.innerWidth <= 768;
  if ((t.classList.add("active"), (document.body.style.overflow = "hidden"), s))
    if (
      ...
      ((e.id = "pdfLoadingMsg"),
        (e.innerText = "Loading PDF viewer..."),
        (e.style.color = "white"),
        ...
        loadScript(
          ...
          const e = document.getElementById("pdfLoadingMsg");
          (e && e.remove(),
            isMobileRendered ||
    ...
    ((i.style.display = "none"),
      ...
      (n.src && n.src !== window.location.href) ||
        (n.src = "document/Harsh_Kumar_Sinha_Resume.pdf#toolbar=0&view=FitH"));
}
function renderMobilePDF(e) {
  initPDFWorker();
  pdfjsLib.getDocument(e).promise.then(
    function (e) {
      ((pdfDoc = e),
        e.getPage(1).then(function (e) {
          const t = document.getElementById("mobileResumeContainer"),
            n = e.getViewport({ scale: 1 }),
            i = (t.clientWidth || window.innerWidth) / n.width,
            s = window.devicePixelRatio || 1,
            o = e.getViewport({ scale: i * s });
          ((canvas.height = o.height),
            (canvas.width = o.width),
            (canvas.style.width = o.width / s + "px"),
            (canvas.style.height = o.height / s + "px"));
          const a = { canvasContext: ctx, viewport: o };
...
function sendMail(e) {
  ...
  const t = document.getElementById("name").value.trim(),
    ...
    i = document.getElementById("subject").value.trim(),
    s = document.getElementById("message").value.trim();
  if (!(t && n && i && s))
  ...
  const o = document.querySelector(".btn-primary"),
    a = o.innerText;
  o.innerText = "Sending...";
  const r = {
  ...
  emailjs
    .send("service_nqhm14v", "template_zh2dwoo", r)
    .then((e) => {
...
function updateDynamicKnowledgeBase() {
  try {
    const e = document.querySelector(".intro p");
    e &&
      (knowledgeBase.summary = `<b>Quick Summary:</b><br>${e.textContent.trim()}`);
    ...
    if (t) {
      const e = Array.from(t.querySelectorAll("p"))
        ...
        .filter((e) => e.length > 0)
        .join("<br><br>");
      e && (knowledgeBase.about = e);
    }
    const n = document.querySelector(".skills-grid");
    if (n) {
      let e = [];
      n.querySelectorAll(".skill").forEach((t) => {
        const n = t.querySelector("h3")?.textContent.trim();
        n && e.push(n);
      });
      (document
        .querySelectorAll(".skills-overview .skill-item span")
        .forEach((t) => e.push(t.textContent.trim())),
        (e = [...new Set(e)]),
        e.length > 0 &&
          (knowledgeBase.skills = `<b>Core Technical Skills:</b><br>${e.join(", ")}`));
    }
    const i = document.querySelector(".experience-section");
    if (i) {
      let e = "<b>Professional Experience:</b><br>",
        t = "<b>Key Projects & Achievements:</b><br>";
      (i.querySelectorAll(".bento-card").forEach((n) => {
        const i = n.querySelector(".duration")?.textContent.trim();
        if (i) {
          const s = n.querySelector("h3")?.textContent.trim();
          let o =
            n
              ...
              .trim() || "";
          e += `• <b>${s}</b> @ ${o} (${i})<br>`;
          n.querySelectorAll(".achievement-list li div").forEach((e) => {
            const n = e.querySelector("strong")?.textContent.trim(),
              i = e.querySelector("p")?.textContent.trim();
            n && (t += `• <b>${n}</b>: ${i}<br>`);
          });
        }
      }),
        (knowledgeBase.experience = e),
        (knowledgeBase.projects = t));
    }
    const s = document.querySelector(".timeline-container");
    if (s) {
      let e = "<b>Education History:</b><br>";
      (s.querySelectorAll(".timeline-item .content").forEach((t) => {
        const n = t.querySelector("h3")?.textContent.trim(),
          i = t.querySelector("h4")?.textContent.trim(),
          s = t.querySelector("p")?.textContent.trim();
        e += `• <b>${i}</b>: ${s} (${n})<br>`;
      }),
        (knowledgeBase.education = e));
    }
    const o = document.querySelectorAll(".footer-socials a");
    let a =
      "You can reach Harsh via the <a href='#contact' style='color: #a855f7;'>contact form</a> below.<br>";
    (o.length > 0 &&
      ((a += "<b>Social Links:</b><br>"),
      o.forEach((e) => {
        const t = e.href,
          n = e.querySelector("img")?.alt || "Profile";
        a += `• <a href='${t}' target='_blank' style='color: #a855f7;'>${n}</a><br>`;
      }),
      (knowledgeBase.contact = a)),
...
function toggleChatbot() {
  (chatbotWidget.classList.toggle("active"),
    chatbotWidget.classList.contains("active") &&
      setTimeout(() => chatInput.focus(), 300));
}
function handleChatInput(e) {
  "Enter" === e.key && sendUserMessage();
}
function sendUserMessage() {
  const e = chatInput.value.trim();
  e &&
    (addMessage(e, "user-message"),
    (chatInput.value = ""),
    showTypingIndicator(),
    setTimeout(
      () => {
        hideTypingIndicator();
        addMessage(getSmartResponse(e), "bot-message");
      },
      1e3 + 1e3 * Math.random(),
    ));
}
function addMessage(e, t) {
  ...
  (n.classList.add("message", t),
    (n.innerHTML = e),
    chatbotBody.insertBefore(n, typingIndicator),
    scrollToBottom());
}
...
"requestIdleCallback" in window
  ? requestIdleCallback(() => updateDynamicKnowledgeBase(), { timeout: 4e3 })
  : setTimeout(() => updateDynamicKnowledgeBase(), 4e3);
const getGeminiApiKey = () =>
  ["EWECuxY", "nC50f6", "Vo2FHF", "mYHlcF", "KLQLJhM", "BySazIA"]
...
async function sendUserMessage() {
  ...
  if (e) {
    (addMessage(e, "user-message"),
      ...
      showTypingIndicator());
    try {
      const t = await getSmartResponse(e);
      (hideTypingIndicator(),
        addMessage(t, "bot-message"),
        jarvis.isWakeMode && jarvis.speak(t));
    } catch (t) {
      (console.error("Error getting response:", t),
        hideTypingIndicator(),
        addMessage(
        ...
        setTimeout(() => {
          (addMessage(getLocalResponse(e), "bot-message"),
            jarvis.isWakeMode && jarvis.speak(getLocalResponse(e)));
        }, 1e3));
    }
  }
}
async function getSmartResponse(e) {
  ...
  try {
    ...
    if (n.error) throw new Error(n.error.message || "API Error");
    if (n.candidates && n.candidates.length > 0) {
      const e = n.candidates[0];
      if ("SAFETY" === e.finishReason)
      ...
      if (e.content && e.content.parts && e.content.parts.length > 0) {
        let t = e.content.parts[0].text;
        return ((t = t.replace(/\*\*(.*?)\*\*/g, "<b>$1</b>")), t);
      }
    }
    throw (
      console.warn("Unexpected API Response:", n),
      new Error("Invalid API response structure")
    );
  } catch (e) {
    throw (console.error("Gemini API Request Failed:", e), e);
  }
}
function getLocalResponse(e) {
  const t = e.toLowerCase();
  return t.includes("summary") ||
    t.includes("overview") ||
    t.includes("tell me about harsh")
    ...
    : t.includes("skill") ||
      ...
      : t.includes("experience") ||
        ...
        : t.includes("education") ||
          ...
          : t.includes("project") || t.includes("built") || t.includes("create")
            ...
            : t.includes("contact") ||
              ...
              : t.includes("resume") || t.includes("cv")
                ? "You can view or download the resume by clicking the <b>'My Resume'</b> button at the top of the page."
                : t.match(/\b(hi|hello|hey|greetings|start)\b/)
                  ? knowledgeBase.greetings[
                      Math.floor(Math.random() * knowledgeBase.greetings.length)
                    ]
                  : t.includes("loader") || t.includes("loading")
                    ? "That's a custom preloader built with CSS animations! It adds a touch of uniqueness before showcasing the main content."
                    : t.includes("moving") ||
                        t.includes("mouse") ||
                        t.includes("effect")
                      ? "That's a 3D Parallax effect in the hero section! It reacts to your mouse movement to create a sense of depth."
                      : "That's a great question! While I'm a simple bot, I can tell you about Harsh's <br>• <b>Experience</b><br>• <b>Skills</b><br>• <b>Education</b><br>• <b>Projects</b><br><br>Or ask for a <b>Summary</b>!";
}
class JarvisAssistant {
    ...
    if (
      ...
      ((this.recognition = new e()),
        ...
        (this.recognition.onstart = () => {
          (console.log("Voice Recognition Started"), this.updateUI());
        }),
        (this.recognition.onend = () => {
          if (
            (console.log("Voice Recognition Ended"),
            this.isWakeMode && !this.isSpeaking)
          )
            try {
            ...
            this.awake
              ? ((this.commandBuffer = t),
                ...
                (this.commandTimer = setTimeout(() => {
                  this.commandBuffer.trim() &&
                    (this.processCommand(this.commandBuffer),
                    (this.commandBuffer = ""));
                }, 1500)))
              : (t.includes("hello jarvis") ||
                  t.includes("hello service") ||
                  t.includes("hey jarvis")) &&
                this.wakeUp());
        }),
        (this.recognition.onerror = (e) => {
          (console.error("Speech Recognition Error:", e.error),
            "not-allowed" === e.error && this.stop());
        }));
    } else {
      console.warn("Web Speech API not supported in this browser.");
      const e = document.getElementById("micBtn");
  ...
  toggle() {
    this.isWakeMode ? this.stop() : this.start();
  }
  start() {
    if (this.recognition) {
      ((this.isWakeMode = !0), (this.awake = !1));
      try {
        (this.recognition.start(),
  ...
  speak(e) {
    ...
    const i =
      this.voices.find(
        (e) =>
          e.name.includes("David") ||
          e.name.includes("Mark") ||
          e.name.includes("Male"),
      ) ||
      this.voices.find((e) => e.name.includes("Google US English")) ||
      this.voices.find((e) => "en-US" === e.lang);
    (i && (n.voice = i),
      (n.onend = () => {
        if (((this.isSpeaking = !1), this.isWakeMode))
          try {
            this.recognition.start();
          } catch (e) {
            console.log("Resume listening...");
          }
        this.awake && this.startSilenceTimer();
      }),
      this.synth.speak(n));
  }
  startSilenceTimer() {
  ...
  updateUI() {
    const e = document.getElementById("micBtn"),
      t = document.getElementById("chatStatus"),
      n = document.getElementById("voiceStatus");
    this.isWakeMode
      ? (e.classList.add("active"),
        this.awake
          ? (e.classList.add("listening"),
            ...
            (n.style.display = "block"),
            (n.innerText = "Listening..."),
            (n.style.color = "#ef4444"))
          : (e.classList.remove("listening"),
            (t.style.display = "none"),
            (n.style.display = "block"),
            (n.innerText = "Say 'Hello Jarvis'"),
            (n.style.color = "#a855f7")))
...
class ThemeManager {
  ...
  init() {
    (window.innerWidth <= 768 && (this.storedTheme = "system"),
      ...
      window
        ...
        .addEventListener("change", (e) => {
          "system" === this.storedTheme && this.applyTheme("system");
        }));
  }
  setTheme(e) {
    ((this.storedTheme = e),
  ...
  setActiveButton(e) {
    (this.buttons.forEach((t) => {
      (t.classList.remove("active"),
        t.dataset.theme === e && t.classList.add("active"));
    }),
      this.themeToggle && this.themeToggle.setAttribute("data-active", e));
  }
  applyTheme(e) {
    let t = e;
    if ("system" === e) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const e = document.getElementById("preloader"),
    t = document.getElementById("loader-circle-fill"),
    n = document.getElementById("loader-percentage");
  if (e) {
    let e = 0;
    const i = 12;
    !(function s() {
      ...
      if (
        ...
        (e += o),
        e > 100 && (e = 100),
        n && (n.innerHTML = `${e}<span class="loader-pct">%</span>`),
        t)
      ) {
        const n = 2 * Math.PI * 46,
          i = n - (e / 100) * n;
        t.style.strokeDashoffset = i;
      }
      if (e < 100) {
        setTimeout(s, e > 80 ? i + 25 : i);
      } else
        setTimeout(() => {
...
class TypingEffect {
  constructor() {
    ...
    const e = new IntersectionObserver((e) => {
      e.forEach((e) => {
        this.isPaused = !e.isIntersecting;
      });
    });
    this.textElement &&
      (e.observe(document.querySelector(".intro")), this.type());
  }
  type() {
    if (this.isPaused) return void setTimeout(() => this.type(), 500);
    const e = this.wordIndex % this.words.length,
      t = this.words[e];
    (this.isDeleting
    ...
    let n = this.typeSpeed;
    (this.isDeleting || this.txt !== t
      ? this.isDeleting &&
        "" === this.txt &&
        ((this.isDeleting = !1), this.wordIndex++, (n = 500))
      : ((n = 2e3), (this.isDeleting = !0)),
      setTimeout(() => this.type(), n));
  }
}
function initFloatingNav() {
  const e = document.querySelectorAll("section, .intro"),
    t = document.querySelectorAll(".dot-nav .dot-nav-item");
  if (!t.length) return;
      ...
      requestAnimationFrame(() => {
        e.forEach((e) => {
          if (e.isIntersecting) {
            ...
            if (n) {
              const e = document.querySelector(`.dot-nav a[href="#${n}"]`);
              if (e) e.classList.add("active");
              else if ("home" === n) {
                const e = document.querySelector('.dot-nav a[href="#"]');
